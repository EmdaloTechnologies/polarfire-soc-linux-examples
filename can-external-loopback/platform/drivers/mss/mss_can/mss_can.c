...
)
{
    ...
    for (mailbox_number = 0u; mailbox_number < CAN_RX_MAILBOX; mailbox_number++)
    {
        ret_value = MSS_CAN_config_buffer_n(this_can, mailbox_number, 
                                            &canrxobj);
	if ((ret_value != CAN_OK) && (ret_value != CAN_BASIC_CAN_MAILBOX)) {
		return ret_value;
	}
    }
    ...
    if (CAN_SPEED_MANUAL == bitrate)
    {
        ...
        {
		fprintf(stderr, "tseg1 too small");
        ...
        if ((pcan_config->CFG_TSEG2 == 0u) ||
            ((pcan_config->SAMPLING_MODE == 1u) && (pcan_config->CFG_TSEG2
             == 1u)))
        {
		fprintf(stderr, "tseg2 too small");
...
)
{
	...
	//this_can->hw_reg->TxMsg[mailbox_number].TXB.L = pmsg->L;
	//asm("fence");
	//this_can->hw_reg->TxMsg[mailbox_number].TXB.L = pmsg->L | CAN_TX_WPNH_EBL;
	//asm("fence");
	//this_can->hw_reg->TxMsg[mailbox_number].TXB.L = pmsg->L | CAN_TX_WPNH_EBL | CAN_TX_REQ;
	//asm("fence");
        return (CAN_VALID_MSG);
    }
    else
...
#else
    printf("Unexpected MSS CAN interrupt - MSS CAN NVIC Interrupts should be \
           disabled");
#endif
    return 0;
}
