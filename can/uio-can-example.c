...
/*
 *	Simple UIO CAN Example -- Send and Receive one message in local loopback mode
 *
...
int wait_for_interrupt(int uiofd, volatile struct can_device * dev)
{
	struct pollfd fds = {
		 .fd = uiofd,
	 	.events = POLLIN,
	...
	if (ret != 0) {
		if (fds.revents & POLLIN) {
			read(uiofd, &reenable, sizeof(int));
			val = can_get_int_status(dev);
			handle_int(dev, val);
			write(uiofd, &reenable, sizeof(int));
		}
	}
...
void runtest(volatile struct can_device *dev, char *devname)
{
	...
	printf("setting can device at %s into local loopback mode\n", devname);
